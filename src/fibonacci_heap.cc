...
std::vector<Item> FibonacciHeap::delete_k(unsigned k) {
  ...
  unsigned max_element = result.size() - 1;
  if (med_sel.size() == 0) {
    max_element = std::max_element(result.begin(), result.end()) - result.begin();
  }
  ...
  for (auto node : result_nodes) {
    if (!(result[max_element] < node->value)) {
